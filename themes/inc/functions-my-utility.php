...
/****************** ▽For HTML useful function *******************/
function my_appropriate_make_ipt_tag($itype, $attrs){
	$ipt_tag = '';
	switch( $itype ){
		case 'select':
			$values = array();
			...
			if( isset($attrs['_value']) ){
				$_val = $attrs['_value'];
				unset($attrs['_value']);
			}
			$ipt_tag = my_make_select_tag($attrs, $values, $_val);
			break;
		...
		case 'textarea':
			...
			$ipt_tag = my_make_textarea_tag($attrs, $val);
			break;

		default:
			$ipt_tag = my_make_ipt_tag($attrs);
			break;
	}
	return $ipt_tag;
}

function my_make_ipt_tag($attrs){
	return my_make_html_tag('<input', $attrs);
}

function my_make_textarea_tag($attrs, $value=""){
	return my_make_html_tag('<textarea', $attrs, '>'.esc_html($value).'</textarea>');
}

function my_make_select_tag($slct_attrs, $vals=array(), $value=""){
	$slct_tag = '';
	if( is_array($vals) && $vals ){
		foreach( $vals as $k => $v ){
			...
			if( (string)$value === (string)$k ) $opt_attrs['selected'] = 'selected';

			$slct_tag .= my_make_html_tag('<option', $opt_attrs, '>'.esc_html($v).'</option>');
		}
	}
	$slct_tag = my_make_html_tag('<select', $slct_attrs, '>'.$slct_tag.'</select>');
	return $slct_tag;
}

function my_make_html_tag($start, $attrs=array(), $end='>'){
	$tag = $start;
	if( $attrs ){
		foreach( $attrs as $k => $v ){
			if( is_array($v) ) continue;

			$tag .= sprintf(' %s="%s"', $k, esc_attr($v));
		}
	}
	$tag .= $end;
	return $tag;
